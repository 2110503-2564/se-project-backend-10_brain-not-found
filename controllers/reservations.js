...
//@route Post /api/v1/hospital/:massageShopId/reservations/
//@access Private
exports.addReservation = async (req,res,next) => {
    try {
        req.body.reservation = req.params.reservationId;

        const reservation = await MassageShop.findById(req.params.massageshopId);
        ...
        const existedMassageShops = await Reservation.find({user:req.user.id});
       ...
       return res.status(500).json({success: false, message: 'Can not create appointment'});
    }
};
...
exports.updateReservation = async (req,res,next) => {
    ...
    return res.status(500).json({success: false, message: "Cannot uodate Appointment"});
    }
};
...
//@access Private
exports.deleteResuervation = async (req,res,next) => {
    try{
        let appointment = await Appointment.findById(req.params.id);
